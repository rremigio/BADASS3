...
def check_comp_options(input,verbose=False):
	...
	if not input:
		output={
			...
			"fit_host"		 : True, # host template
			"fit_power"		: True, # AGN power-law
			"fit_poly"		: False, # additive or multiplicative polynomial
			"fit_agn"		: False, # prepared AGN template
			"fit_narrow"	   : False, # narrow lines
			"fit_broad"		: False, # broad lines
			"fit_outflow"	  : False, # outflow lines
			"fit_absorp"	   : False, # absorption lines
...
def check_agn_temp_options(input, verbose=False):
    """
    Checks the inputs. For this, all you really need is a single dictionary.
    """
    
    output={} #output dictionary
    if not input:
        output={
				"galaxy" : "ark120" # just put this in since I know I made this template
		}
        return output
    # check the keyword dictionaries
    keyword_dict = {
	"galaxy"			:{	"conds":[ lambda x: isinstance(x,(str))],
              				"default": "ark120",
                  			"error_message": "\n galaxy needs to be a string. \n",}
	}
    output = check_dict(input, keyword_dict)
